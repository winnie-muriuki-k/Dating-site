...
<section class="latest_news_area p_120 " style="padding-bottom: 0px !important; background: #ded0e4 !important;">
    <div class="l_news_item col-md-10 main-holder">
        <div class="row">
            <div class="col-md-8">
                <h4 style="color: #403a3a;">Who's favourited  you</h4>
            </div>
            <div class="col-md-4">
                <h4 style="color: #403a3a;  color:black; font-size: 15px;">Related Searches <u><a href="{{route('user-favourites')}}" class="">My Favourites</a> </u></h4>
            </div>
        </div>
        ...
        <div class="row">
            @if(count($favourited)>0)
                @foreach($favourited as $fav)
                    <div style="border: 1px solid green; margin: 0px 10px 10px 10px" class="col-md-2 col-sm-6" id="div-match-{{$fav->initiator_id}}">
                        <!------start------>
                        <!------start------>
                        <div class="card" style="margin-bottom: 2%;" id="profile-display-cards">
                            <img src="{{asset('avatars/')}}/{{App\User::findOrFail($fav->initiator_id)->avatar}}"  id="user-profile-image"
                                 alt="Profile Image">
                            <div class="card-body" style="padding: 0px !important;">
                                <p class="card-text text-center" style="margin: 1px !important;">
                                    {{ !empty(App\User::findOrFail($fav->initiator_id)->username) ? App\User::findOrFail($fav->initiator_id)->username : 'No name' }}, <i>&nbsp;{{App\User::findOrFail($fav->initiator_id)->age }}&nbsp;yrs</i>
                                </p>

                                <div style="text-align: center;padding-bottom: 5px;">
                                    <button type="button" class="btn btn-info btn-sm">
                                        <a style="color: white;" href="{{route('view-user',['id'=>$fav->initiator_id])}}"> <i class="fa fa-eye user-information" style="    font-size: 20px;" aria-hidden="true" id="view-more-user-information" title="View profile and interests"
                                            ></i></a>
                                    ...
                                    <button type="button" class="btn btn-info btn-sm" onclick="populateAndShowMessageDiv('{{$fav->initiator_id}}','{{App\User::findOrFail($fav->initiator_id)->username}}','{{Auth::user()->id}}')" id="send-message-btn">
                                        <i class="fa fa-send" title="message {{ !empty(App\User::findOrFail($fav->initiator_id)->username) ? App\User::findOrFail($fav->initiator_id)->username: '' }}" style="    font-size: 20px;" aria-hidden="true" onclick="">
                                    ...
                                    <button type="button" class="btn btn-info btn-sm" onclick="Matchuser('{{Auth::user()->id}}','{{$fav->initiator_id}}')" id="favorite-user-button">
                                        <i title="like {{ !empty(App\User::findOrFail($fav->initiator_id)->username) ? App\User::findOrFail($fav->initiator_id)->username : '' }}" class="fa fa-heart favorite-icon" id="fa-heart-{{$fav->initiator_id}}" style="font-size: 20px;" aria-hidden="true" onclick=""></i>

                                    </button>

                                    &nbsp;
                                    @if(!in_array($fav->initiator_id, $fav_id_array))
                                        <button type="button" class="btn btn-info btn-sm" onclick="addFav('{{Auth::user()->id}}','{{$fav->initiator_id}}')" id="add_fav_{{$fav->initiator_id}}">
                                            <i title="Add {{ !empty(App\User::findOrFail($fav->initiator_id)->username) ? App\User::findOrFail($fav->initiator_id)->username : '' }} To Favourites" class="fa fa-thumbs-up" id="fa-heart-{{$fav->initiator_id}}" style="font-size: 20px;" aria-hidden="true" onclick=""></i>
