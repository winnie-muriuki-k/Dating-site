...
<section class="latest_news_area p_120 " style="padding-bottom: 0px !important; background: #ded0e4 !important;">
    <div class="l_news_item col-md-10 main-holder">
        <div class="row">
            <div class="col-md-8">
                <h4 style="color: #403a3a;">Whom I'm interested in </h4>
            </div>
            <div class="col-md-4">
                <h4 style="color: #403a3a;  color:black; font-size: 15px;">Related Searches <u><a href="{{route('interested',['id'=>Auth::user()->id])}}" class="">Interested In Me</a> </u></h4>
            </div>
        </div>
        ...
        <div class="row offset-md-2">
            @if(count($interests)>0)
                @foreach($interests as $inter)
                    <div class="col-md-3" id="div-match-{{$inter->user_recipient->id}}">
                        <div class="l_news_item member-profile-holder">
                            <div class="row">
                                <img src="{{asset('avatars/')}}/{{$inter->user_recipient->avatar}}" class="img-responsive" alt="" style="width: 150px;height: 150px;border: 1px solid #bebebe; margin: auto;">

                            </div>


                            <div class="post_view row" style="display: inline-block;">
                                <div style="margin-bottom: 5px;">
                                    <span style="font-weight: bold;">{{ !empty($inter->user_recipient->username) !==null ? $inter->user_recipient->username : 'No name' }}, &nbsp;{{$inter->user_recipient->age}}</span>
                                </div>
                                ...
                                <div>
                                    <i class="fa fa-user user-information" style="    font-size: 20px;margin-right: 5px;" aria-hidden="true" id="view-more-user-information" title="View profile and interests"
                                       onclick="showUserInformationOntheModal('{{$inter->user_recipient->id}}')"></i>


                                    <i class="fa fa-send" title="message {{ !empty($inter->user_recipient->username) !==null ? $inter->user_recipient->username : '' }}" style="    font-size: 20px;color: #57a700;margin-right: 5px;" aria-hidden="true" onclick="populateAndShowMessageDiv('{{$inter->user_recipient->id}}','{{$inter->user_recipient->username}}','{{Auth::user()->id}}')">

                                        {{-- populateAndShowMessageDiv(receiver_id,receiver_username,sender_id) --}}
                                    </i>

                                    <i title="like {{ !empty($inter->user_recipient->username) !==null ? $inter->user_recipient->username : '' }}" class="fa fa-heart favorite-icon" id="fa-heart-{{$inter->user_recipient->id}}" style="font-size: 20px;" aria-hidden="true" onclick="Matchuser('{{Auth::user()->id}}','{{$inter->user_recipient->id}}')"></i>
                                </div>
